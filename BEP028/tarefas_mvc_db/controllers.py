...
class TarefaController:
    ...
    def __init__(self, repository, view):
        ...
        Args:
            repository: Instância de TarefaRepository
            view: Instância de TarefaView
        """
    ...
    def criar_tarefa(self):
        ...
        try:
            ...
            tarefa_id = self.repository.criar(tarefa)
            if tarefa_id:
                self.view.exibir_mensagem("Tarefa criada!", "sucesso")
            else:
                self.view.exibir_mensagem("Erro ao criar tarefa!", "erro")
    ...
    def marcar_concluida(self):
        ...
        if tarefa:
            ...
            if self.repository.atualizar(tarefa):
                self.view.exibir_mensagem("Tarefa concluída!", "sucesso")
            else:
                self.view.exibir_mensagem("Erro ao atualizar tarefa!", "erro")
        else:
    ...
    def remover_tarefa(self):
        ...
        tarefa = self.repository.buscar_por_indice(indice)
        if tarefa:
            tarefa_removida = self.repository.remover(tarefa.id)
            if tarefa_removida:
                self.view.exibir_mensagem("Tarefa removida!", "sucesso")
            else:
                self.view.exibir_mensagem("Erro ao remover tarefa!", "erro")
        else:
            self.view.exibir_mensagem("Tarefa não encontrada!", "erro")
