...
app = FastAPI(title="API Tarefas (JWT + Logs + SQLite + Erros)")
...
def resposta_erro(status: int, mensagem: str, erros: dict | None = None):
    payload = {"status": status, "mensagem": mensagem}
    if erros:
        payload["erros"] = erros
    return payload
...
def validar_token(token: str):
    try:
        ...
        if exp_ts is None:
            raise HTTPException(
                status_code=401,
                detail=resposta_erro(401, "Token sem exp"),
            )
        if datetime.now(timezone.utc).timestamp() > exp_ts:
            raise HTTPException(
                status_code=401,
                detail=resposta_erro(401, "Token expirado"),
            )
        return dados
    except JWTError:
        raise HTTPException(status_code=401, detail=resposta_erro(401, "Token invalido"))
...
def login(dados: dict = Body(...)):
    ...
    if usuario != "admin" or senha != "123":
        raise HTTPException(
            status_code=401,
            detail=resposta_erro(401, "Credenciais invalidas"),
        )
...
def buscar_tarefa(tarefa_id: int, token: str = Depends(oauth2_scheme)):
    ...
    with obter_conexao() as conn:
        ...
        if not linha:
            raise HTTPException(
                status_code=404,
                detail=resposta_erro(404, "Tarefa nao encontrada"),
            )
        return {
            "id": linha[0],
            "descricao": linha[1],
...
def criar_tarefa(tarefa: dict = Body(...), token: str = Depends(oauth2_scheme)):
    ...
    if not descricao or not isinstance(descricao, str):
        erros["descricao"] = "Descricao obrigatoria"
    if not prioridade or not validar_prioridade(prioridade):
        erros["prioridade"] = "Prioridade invalida"
    if erros:
        raise HTTPException(status_code=400, detail=resposta_erro(400, "Dados invalidos", erros))
...
def atualizar_status(
    ...
    if status is None or not isinstance(status, (int, float)):
        erros["status"] = "Status deve ser numero"
    elif status < 0 or status > 100:
        erros["status"] = "Status deve ser 0 a 100"
    if erros:
        raise HTTPException(status_code=400, detail=resposta_erro(400, "Dados invalidos", erros))
